...
def test_the_boundary_follows_setup_and_covers_every_test_and_coverage_step(jobs, name):
    ...
    for index, step in consumers:
        ...
        expected = (
            ...
            else "${{ runner.os == 'Linux' && '/usr/local/bin/ci-shell {0}' || 'bash' }}"
        )
        assert step["shell"] == expected
...
def test_all_linux_shards_and_memory_heavy_jobs_use_large(jobs):
    ...
    assert lint["env"]["BLACK_NUM_WORKERS"] == "4"
    assert lint["run"] == "python3 scripts/check_black_formatting.py"
...
def test_privilege_transition_only_goes_down_and_temp_stays_root_owned(action):
    ...
    assert "exec runuser -m -u runner -- env HOME=/home/runner USER=runner LOGNAME=runner" in code
