...
@pytest.mark.asyncio
async def test_export_scrubs_message_prose_but_keeps_fenced_code_verbatim():
    """The ruled scope is message prose only: a fenced code block remains
    byte-for-byte for re-use, while paths before and after it are redacted."""
    code = f"```python\nsource = {_PATH!r}\n```"
    msgs = [
    ...
    for message in document["messages"]:
        assert message["content"].count("[redacted-path]") == 2
        assert code in message["content"]
        assert f"source = {_PATH!r}" in message["content"]
...
async def test_export_scrubs_inline_and_unclosed_backtick_spans():
    """Only a closed, line-anchored Markdown fence is preserved.

    Inline triple backticks and an unclosed fence are prose for export privacy,
    so their paths are scrubbed rather than shipped verbatim.
    """
    msgs = [
