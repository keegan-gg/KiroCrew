...
class TestSuccessfulParse:
    ...
    def test_noul_answer_puts_the_probability_in_both_value_and_p(self):
        ...
        answer = asyncio.run(_run(rec, [NOUL]))["is_urgent"]
    ...
    def test_a_usage_block_is_read_past_rather_than_parsed(self):
        ...
        """
        with_usage = _Recorder(
            body=_ok_body({"is_urgent": {"type": "noul", "noul": 0.1}}, input_tokens=1900)
        )
        without = _Recorder(body={"model": "jev-latest", "answers": {"is_urgent": {"noul": 0.1}}})
        first = asyncio.run(_run(with_usage, [NOUL]))
        second = asyncio.run(_run(without, [NOUL]))
        assert set(first) == set(second) == {"is_urgent"}
        assert first["is_urgent"].value == second["is_urgent"].value == pytest.approx(0.1)

    def test_a_2xx_other_than_200_is_accepted(self):
        rec = _Recorder(status=202, body=_ok_body({"is_urgent": {"type": "noul", "noul": 0.3}}))
        assert asyncio.run(_run(rec, [NOUL]))["is_urgent"].value == pytest.approx(0.3)
