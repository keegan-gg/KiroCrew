"""``DecisionsConfig``: what enables the seam, and everything that must not.

``enabled`` is the one value that lets a decision point send conversation state to
a third party, so the tests here are written from the direction of the risk: the
default is off, only the literal ``true`` opens it, and every unreadable or
legacy-shaped value resolves to off rather than to a guess.

The migration group is the reason this file exists separately from the gate's.
An operator may still hold the earlier ``preview`` + ``points.<name>.arm``
spelling in ``config.json``; those values expressed intent against a DIFFERENT
switch, so inferring ``enabled`` from them would turn the seam on from something
nobody wrote for it.
"""
...
class TestDefaults:
    def test_the_seam_is_off_and_fully_sampled_by_default(self):
        cfg = DecisionsConfig()
        assert cfg.enabled is False
        assert cfg.bucket == DECISION_BUCKET_MAX

    def test_the_section_is_three_fields(self):
        """The arm/impl/points vocabulary is retired; a re-added key lands here."""
        from dataclasses import fields

        assert {f.name for f in fields(DecisionsConfig)} == {"enabled", "bucket", "provider"}
...
class TestEnable:
    def test_the_literal_true_enables(self):
        assert DecisionsConfig.from_raw({"enabled": True}).enabled is True

    @pytest.mark.parametrize(
        "raw",
        [1, "true", "True", "yes", "1", 0.5, [1], {"a": 1}, None, "", 0, False],
    )
    def test_nothing_else_enables(self, raw):
        """``bool("false")`` is True, so a truthiness read here would arm the seam."""
        assert DecisionsConfig.from_raw({"enabled": raw}).enabled is False
...
class TestMigrationFromThePreviewSpelling:
    ...
    def test_a_legacy_config_parses_to_off(self, legacy):
        assert DecisionsConfig.from_raw(legacy).enabled is False
    ...
    def test_the_legacy_keys_do_not_survive_a_round_trip(self):
        ...
        assert set(saved) == {"enabled", "bucket", "provider"}
...
class TestBucket:
    ...
    def test_an_unreadable_bucket_falls_back_to_full_sampling(self, raw):
        """``enabled`` is the switch; a malformed bucket must not silently disable."""
        assert DecisionsConfig.from_raw({"bucket": raw}).bucket == DECISION_BUCKET_MAX
...
class TestTheLoaderWiresTheSection:
    def test_a_full_config_load_carries_the_section(self, tmp_path, monkeypatch):
        ...
        assert cfg.decisions.enabled is True
        assert cfg.decisions.bucket == 10
        assert cfg.to_dict()["decisions"]["enabled"] is True
