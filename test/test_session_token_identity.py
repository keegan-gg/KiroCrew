...
def test_kiro_identity_projection_fails_closed_on_settings_errors(
    ...
    if failure == "refused":
        ...
        target = tmp_path / "synthetic-credential.json"
        ...
        try:
            ...
            if getattr(exc, "winerror", None) == 1314:
                pytest.skip("Creating a Windows symlink requires developer mode")
            raise
        monkeypatch.setattr(hooks, "is_sensitive_path", lambda raw: Path(raw) == target)
        monkeypatch.setattr(
