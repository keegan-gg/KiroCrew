...
def test_a_failing_enumeration_offers_nothing():
    class _BadIter(_FakeLoader):
        def _iter_visible(self, project_dir=None):
            raise RuntimeError("tree walk failed")

    assert sel.candidates_from_loader(_BadIter([], {}), "zebra", None) == []
...
def test_a_scheduling_failure_closes_the_coroutine(bg_loop, enabled, monkeypatch, recwarn):
    monkeypatch.setattr(core, "decide", AsyncMock(return_value=_answers("review")))
    ...
    gc.collect()
    assert [w for w in recwarn.list if "never awaited" in str(w.message)] == []
