...
def _expression(value, context):
    """Evaluate only the small, explicitly allowed Actions routing grammar."""
    source = value.removeprefix("${{").removesuffix("}}").strip()
    for key in sorted(context, key=len, reverse=True):
        source = source.replace(key, repr(context[key]))
    ...
    assert all(node.id in functions for node in ast.walk(tree) if isinstance(node, ast.Name))
    return eval(compile(tree, "<runner-expression>", "eval"), {"__builtins__": {}, **functions})
...
def test_all_actual_routes_require_repository_event_and_admitted_actor(
    jobs, repository, head, event, actors, actor
):
    context = {
    ...
    expected = (
        ...
        and (event == "push" or (event == "pull_request" and head == _REPO))
    )
...
def test_shard_selection_and_checks_stay_intact(jobs):
    ...
    assert command == (
        ...
        "-q -n auto --timeout=180 --no-cov --max-worker-restart=0 "
...
def test_probe_executes_owned_file_operations_and_cleans_on_failure(tmp_path, monkeypatch, fault):
    tree = ast.parse((_ACTION / "probe.py").read_text(encoding="utf-8"))
    function = next(
        node
        for node in tree.body
        if isinstance(node, ast.FunctionDef) and node.name == "check_paths"
    )
    pc = SimpleNamespace(
    ...
    exec(
        compile(ast.Module(body=[function], type_ignores=[]), "<windows-probe>", "exec"), namespace
    )
    if fault:
        with pytest.raises((AssertionError, OSError)) as error:
            namespace["check_paths"]()
        if fault == "owner":
...
def test_probe_main_fails_on_missing_native_runtime_or_tools(monkeypatch, fault):
    tree = ast.parse((_ACTION / "probe.py").read_text(encoding="utf-8"))
    ...
    exec(compile(ast.Module(body=[function], type_ignores=[]), "<probe-main>", "exec"), namespace)
    if fault:
