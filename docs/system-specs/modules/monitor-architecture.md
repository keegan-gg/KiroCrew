...
- Superseded attempts collapse to the newest per check identity. A host leaves a
  ...
  conflated. A row is dropped ONLY when its own run was cancelled AND a newer run of
  its identity exists: the rollup carries no lineage edge, so recency alone does not
  license removing a row, while a cancellation by the concurrency group does establish
  displacement and the row carries no verdict to lose. Separately, the NEWEST run being
  ...
  replaced round reads as live, and keying on the run instead of on the conclusion is
  ...
  comparison that picks the newest run. Over-reporting costs a turn; hiding a
