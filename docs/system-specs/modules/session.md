...
entries are cloned rather than merely validated. Materialization happens at
exactly two moments, both owner-initiated: **add** (`GitProjectStore.add` for a
cloned bundle, and the local-bundle path in `handlers_project`, which has no
`sync` to catch up on) and **sync** (`GitProjectStore.sync_sources`, after the
bundle's own fast-forward, using the manifest the pull just delivered). Session
attachment NEVER materializes — that is what keeps an agent edit to
...
A source that cannot be materialized does not fail the add: the Project registers
and its health reports `sources_unavailable` with `unavailable_sources` naming the
failing source ids, so the owner can correct the URL and sync. A missing
...
only when no non-absent discovery surface exists beyond `project.yaml`. An add
with discovery files registers without a review record, reports those files as
added and review-stale, and refuses session start until explicit acceptance.
...
in-tree file — a link to a target outside the checkout, an oversized file — is
...
kiro-cli would follow the link and load a target the digest cannot see. A checkout
...
present in that state alone. `resolve_project_attachment` then refuses with
...
and health: each row carries `path`, `status` (`added`, `changed`, `removed`, or
...
Credential-bearing preview text is redacted for display.
Unreadable reasons are `link-outside-root`, `too-large`, `binary`, `error`, or `overflow`.
...
without recording acceptance. This also makes a failed-at-add local source
retryable without re-adding the Project. A matching, readable snapshot is
SEL-audited before its digest and per-file hashes are recorded. Matching
