...
- **Failed report settlement is boundary-owned and bounded.** Only a report with a non-empty `_stage_boundary_owner` enters the failure latch. Entries are keyed by `(parent_session_key, boundary_owner)`, retain at most 500 scopes, and saturate each count at 65 (64 failures plus an overflow sentinel). If scope capacity is exhausted, evicted and newly dropped failures move into one bounded overflow entry; the next owned boundary fails closed instead of treating lost identity as success. An ordinary non-Autopilot report is unowned, so its failure cannot halt or advance a later stage.
