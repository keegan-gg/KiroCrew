...
A Markdown fenced code block is left byte-for-byte: it may hold a copied script
or command a recipient needs to run, so paths inside a fence are preserved while
every surrounding prose segment is scrubbed. This is **not an opt-in and there
