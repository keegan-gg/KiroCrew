...
1. the **dashboard slot + bell** — `deliver_cron_run` (gated on `cron_run_gets_tab`: `not hide_in_chat and (persistent_session or chat_folder_id)`) then `notify`;
...
A SECOND, unrelated folder field (issue #1620). `folder_id` above groups the job's ROW on the Schedule page (`cron_folders.json`); `chat_folder_id` names a folder in the **chat sidebar's** own tree (`folders.json`) and decides where the job's RUNS land. A job may carry either, both or neither, and neither is ever derived from the other.

**What it is for**: a recurring job's output otherwise lands loose — a notification, a Slack DM, or appended onto one long-lived tab — so there is no way to read the job's runs back as a timeline. With a chat folder set, that folder IS the job's history.

**Which runs are in scope**: exactly those that get a chat session, which is the same set `hide_in_chat` already governs. A `script`/`command` cron takes no agent turn and never creates a slot, so it has nothing to file; the Schedule form therefore does not offer the picker for one and sends `""` — the same reason it omits `minimal_context` there. A job with `hide_in_chat` on is the other case: the form disables the picker, says why, and sends `""`, so a setting whose only observable effect would be a folder that never fills up cannot be stored. `cron_run_gets_tab` is the backend half of the same rule.
...
**Which tab a run lands in** depends on the job's session shape, and both answers come from `cron_inject.deliver_cron_run`, the one delivery chokepoint every site now calls:

- **Persistent job** (`persistent_session=True`, the default): the job's single `cron-{id}` tab is filed into the folder. One entry, which is what a job that keeps one conversation has to offer.
- **Stateless job** (`persistent_session=False`): each run's own session key (`cron:{id}:{run_id}`, minted by `build_cron_session_context`) gets a tab of its own, named by `chat_utils.cron_slot_name` (`cron-{id}-{run_id}`) and filed into the folder — the browsable per-run timeline the issue asks for. Its transcript is written under its OWN key, not `cron:{id}`: hydrating it from the job-wide pile would put back exactly the context `persistent_session=False` removes, so `prefetch_cron_run_history` returns `None` for such a job and the whole-file read is skipped.

A stateless job with NO chat folder is unchanged: it still gets no tab, because a per-run tab with nowhere to live is the sidebar clutter that setting exists to avoid. `hide_in_chat` wins over a configured folder — it is the narrower, older and more emphatic opt-out, and resolving the contradiction toward no-tab cannot surprise anyone.

**Suppressed runs** (dedup, `silent`) keep their existing rule for a persistent job: they re-inject only into a tab that already exists, so suppressed output never conjures one. For a stateless job filing into a folder that rule cannot apply — its tab is per-run, so "already exists" is never true — and applying it would silently drop every deduped or silent run from the timeline the user configured the folder to see. `cron_suppressed_run_gets_tab` holds both answers in one place.

**Pre-create**: `ensure_cron_slot` is deliberately NOT widened. Its gate stays `persistent_session and not hide_in_chat`, so a stateless job's per-run tab is minted at DELIVERY only. Pre-creating one would file a session into the user's folder before the run produced anything, and a run that ends with no result (an error, a cancellation, a governance refusal) would leave an empty session in the timeline for good. The cost is that such a run has no first-run session-control caller identity; the benefit is a folder in which every entry is a run that actually produced output.

**Folder lifecycle.** A RENAME is a no-op — the job stores the folder's id, so it follows the folder under its new name. A DELETE leaves a dangling id, and the contract matches `folder_id`'s: a dangling id means "not filed". The run still delivers, its session lands unfiled, and the skip is recorded ONCE (a log line plus a `cron_chat_folder_missing` SEL row with outcome `skipped`) so "why is this run not in my folder?" has an answer without a repro. Failing the run over a missing folder would trade a cosmetic loss for a lost run, so `file_cron_run_in_chat_folder` and `persist_cron_chat_folder` are both exception-contained.

**Existence is checked at SAVE time**, where a person is present to be told: `POST`/`PATCH /api/crons` refuse an id naming no folder in `state._folders` with `400 unknown_chat_folder` (and a non-string or over-cap value with `400 invalid_chat_folder_id`; `null` means "not filed"). The runtime still treats a dangling id as unfiled, because a folder can be deleted after the save.

**Persistence of the placement.** The folder's browsable timeline is read from the on-disk session list (`list_sessions` carries each session's `folder_id`), so an in-memory assignment alone would vanish on restart and the folder would show nothing. `persist_cron_chat_folder` therefore does a FORCED `save_slot_off_loop` — the same route every other folder-filing surface takes — and only when the filing actually moved something, so a re-file costs no metadata write per delivery.

**Clearing the field** unfiles the job's own tab, and it happens at the SAVE (`unfile_cron_job_tab`, called by `PATCH /api/crons/{id}` after the store commits) rather than at the next delivery. At delivery the two states are indistinguishable: a job with no folder whose tab sits in one looks identical whether this feature put it there or the reader dragged it there by hand, and unfiling on that guess would move a session the reader placed themselves. Only the job-wide tab moves — per-run tabs from earlier runs are history, and today's settings change is no reason to rewrite where last week's runs are filed.

**The live-slot ceiling.** A per-run tab is the one cron surface that mints a slot per FIRE, and `_bind_cron_slot` reaches `get_or_create_slot` below the `MAX_LIVE_SLOTS` check `session_control.create_session` makes — so the bound is asked for in the bind itself (`per_run_tab_would_exceed_the_slot_ceiling`). Past the ceiling the run delivers with NO tab: the bind answers `None`, nothing is written, and the skip is recorded once as a `cron_per_run_tab_skipped` SEL row. The JOB-WIDE tab is never refused — it is created once and reused for the job's whole life, so refusing it at the ceiling would silence an established job's delivery for a reason that has nothing to do with it.

**Notification CTA.** `notify_meta["slot"]` names the tab THIS run landed in (`cron_slot_name(session_key)`), not `cron-{id}`. Keying it on the job-wide name would offer "Continue session" for a tab that does not exist on a stateless job, or fall back to "View last result" while the run sits in a tab the reader could have been taken straight to.

**Routing.** `chat_utils.dashboard_slot_key` answers a per-run key with its OWN tab when that key has a dashboard surface, and falls back to the job-wide tab otherwise. The ordering is load-bearing and the fallback is what keeps a sequential-agent key (`cron:{id}:{agent}` — same shape, durable, and only ever published under the job's linked key) behaving exactly as before.

**Field wiring**: dashboard-only, exactly like `folder_id` — accepted on `POST /api/crons` and `PATCH /api/crons/{id}`, returned by `GET /api/crons`, persisted through `_save()`/`_load()`, threaded through `_build_job()`/`add_job()`/`add_job_async()`/`_update_job_locked()`, and capped in `_CRON_STRING_FIELD_CAPS`. There is no `cron_add`/`cron_update` (MCP) argument for it, which is why `test_cron_string_field_validation.py` lists it under `_NO_SCHEMA_FIELDS`.

**Retention is deliberately out of scope.** A stateless job filing every run mints one session per run, and those follow the existing session archival like any other session — this adds no pruning mechanism of its own.
