...
- **Flow**: Download to temp file → sniff the raster type → promote into the dashboard's uploads directory (`kiro_crew.uploads.create_upload_file`: `<data_home>/uploads/<uuid hex>_<sanitized name>.<sniffed ext>`, `O_EXCL`, `0o600`, all-or-nothing write, created relative to a pinned directory descriptor through `pinned_fs.create_and_open_dir_pinned` on POSIX, and on Windows under a `platform_compat.pin_directory` handle held across the by-name open and the whole write, so a link planted at `uploads/` is refused rather than followed and the directory cannot be swapped between the check and the last byte) → `append_attachment_context()` writes the row as `![image](/abs/uploads/…)` — the dashboard composer's own shape, with the destination spelled by `uploads.markdown_image_dest` (the mirror of the composer's `mdImageDest`: forward slashes for a Windows path, a `<…>` wrap with `%` written as `%25` for a space or `%`) and decoded again by the outbound extractor's `_finish_destination` (the twin of `mdImageDestToPath`: only the wrapped form is percent-decoded, a bare destination stays verbatim) — so the transcript renders the picture after the turn — and `_send_prompt()` finds the path inside that markup, base64-encodes it, and sends it as a `{"type": "image"}` content block to kiro-cli. A message bound for a linked dashboard slot always promotes (`events._inbound_images_persist`): that turn runs as its own task after this handler's cleanup has deleted every temp file, and the dashboard persists a session's own pasted pictures in every privacy mode, so its inbound ones land in the same place. Accepted residual: a promoted picture in a restricted (incognito/temporary) linked session outlives the conversation, and the uploads directory's own retention is the only control -- the same residual `/api/upload` already carries for pasted pictures; a per-turn cleanup or a retention rule covering both is tracked in kirodotdev/KiroCrew#10871. Within one `data_home` (one principal), `uploads/` is one flat directory approved as an outbound root for every Slack-linked session, so a picture promoted by one linked session is referenceable by another linked session of the same principal; per-session namespacing is tracked in issue #10871 (item 2). Promotion is skipped for a Slack-native thread whose conversation persists nothing — a durable `temporary`/`incognito` flag, or a `!incognito`/`!temporary` token in the message itself — and the image then stays a temp file on the lifecycle below.
- **Temp lifecycle** (opaque files, and images the privacy gate kept temporary): Caller (`_route_message`) owns cleanup. Done callback on `handle_message` task cleans up after `_send_prompt()` reads the file. Early-return paths and `create_task` failures also clean up. Queued messages carry their paths in the entry's `image_temp_paths` kwargs; `_dispatch_queued` unlinks after the turn consumes them, and the queue-discard paths — `cancel_queued`, `clear_queue`, `dequeue`'s cancelled-skip, and the `_pending_queue` drops in `_handle_message_deleted` and the `!stop` handler — unlink via `session.unlink_queued_temp_paths()` so entries that never dispatch don't leak files. A promoted image is listed in `IngestResult.persisted_paths` and excluded from `temp_paths`: it belongs to the transcript, and its retention is the uploads directory's. Known gap: session-teardown paths (restart/remove/destroy/idle sweep) drop `session.queue` without unlinking.
...
- **Slack → Dashboard**: `handle_message()` checks `_slack_to_slot` reverse lookup; if linked, `maybe_route_linked_thread()` routes the message to the dashboard slot's `_run_chat()` (or its queue) tagged `_ingress="slack"` / `ingress="slack"`, so the turn knows the message already sits in the thread
- **Dashboard → Slack**: `_run_chat()` builds the session's `SlackRenderer` through `dashboard/slack_mirror.open_slack_mirror()` (`decider=None`, `reactions_enabled=False`, `user_id=state.owner_id`, `uploads_allowed=not slot.is_restricted`) and drives its `on_turn_start` / `on_text_chunk` (through a turn-scoped `StreamRedactor`) / `on_thinking` / `on_tool_call` / `on_done` from its own event sites, with `close()` on every exit path — the same renderer a Slack-born turn gets from `transport_dispatch`. For each tool card, `SlackRenderer.on_tool_call` applies `redact_for_target` to the full title and purpose before `TOOL_CARD_LIMIT` bounds the redacted values, so both callers share the redact-then-bound order. Two dashboard-only entry points: `echo_user_message()` (the `💬 _…_` echo, skipped for a synthetic turn, a slash turn, or a Slack-ingress turn; pictures extracted and uploaded) and `post_history_row()` (the `🧑`/`🤖` link-time backfill row, used by `chat_slack.drain_slack_backfill`). One pause gate (`slack_mirror_is_paused`) silences echo, stream, reply and teardown together; the approval prompt is still posted once via `post_linked_approval`.
...
- **`SlackRenderer` + `SlackApprovalDecider`** (`slack/renderer.py`): renders abstract output onto a Slack thread for BOTH ingress kinds — driven by `TurnDriver` for a Slack-born turn and by the dashboard turn loop for a dashboard-born turn on a linked session (see Linked Thread Sync). Outbound image uploads read from the roots `authorize_upload_root(cwd)` admits: the session cwd and the dashboard uploads directory together, with the descriptor-pinned read bound to whichever root the reference sits under; an invalid cwd admits neither, and a restricted session (`uploads_allowed=False`) ships no bytes. Approval buttons use `mc_tool_approve_` / `mc_tool_trust_` (per-session Trust) / `mc_tool_deny_` action prefixes. `SlackApprovalDecider` maintains a process-global `_REGISTRY` keyed by request id so the module-level interaction handler can `resolve_global()` a click without a direct reference to the per-turn decider; `session_for()` maps a click back to its session for per-session Trust. The decider is **deny-by-default** — it `wait_for`s the button future and returns `False` on timeout.
