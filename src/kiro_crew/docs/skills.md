...
`~/.kiro/crew/skills/github/<address>/` and you own the result from then on. It is
...
An import is all-or-nothing. If any file cannot be read, is not text, is too
large, or would collide with another on a filesystem that ignores case, the whole
import is refused rather than landing a skill that is missing part of itself.
