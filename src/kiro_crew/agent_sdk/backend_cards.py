...
OFF_CARD_SETS: Mapping[str, str] = {
    ...
    "ACP_BACKENDS_TOOL_SEARCH_OVERLAY": (
        "which harnesses read the Tool Search keys from the cli.json overlay. A channel "
        "detail: KAS takes the same setting over the wire, so the feature reaches the "
        "session either way"
    ),
    "ACP_BACKENDS_CLIENT_META_SETTINGS": (
        "whether a harness takes client settings in initialize's _meta. The other "
        "channel is the overlay file; the setting lands either way, and a wrong "
        "membership is a setting that does not, which is a defect"
    ),
