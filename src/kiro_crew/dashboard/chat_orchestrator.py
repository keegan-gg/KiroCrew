...
async def _release_cancelled_plan_boundary(
    ...
    """Discard one plan's queue debt, then expose the slot as boundary-free."""
    ...
    if not owed or state.subagents is None:
        return
    try:
...
async def _exit_cancelled_plan(state: "DashboardState", slot: "_ChatSlot") -> None:
    ...
    await _release_cancelled_plan_boundary(state, slot)
...
async def _settle_stage_delivery(
    ...
    *,
    ...
    while not _orchestration_stopped(
        ...
        delivery_entry = next(
            (entry for entry in slot._queue if entry.get("kind") in STAGE_DELIVERY_KINDS),
            None,
        )
...
async def api_chat_plan_action(request: web.Request) -> web.Response:
    ...
    if action == "cancel":
        ...
        # must release its rows and retention debt here or ordinary input stays
        # blocked forever behind a revoked stage.
