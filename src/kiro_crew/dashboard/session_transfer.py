...
# File-export path scrubbing applies to message prose, not Markdown fenced code:
# code blocks may contain copied scripts or commands a recipient needs to run.
# Keep those spans byte-for-byte while redacting paths in every surrounding body
# segment. Only closed, line-anchored fences are preserved; an inline or
# unterminated backtick span is ordinary prose and is scrubbed. The closing
# fence line may carry only whitespace after its backticks, so a path written
# after a closing ``` on the same line is not swallowed into the kept span.
...
def _redact_file_export_message_paths(content: str) -> str:
    """Redact path prose while preserving fenced code blocks verbatim."""
    parts: list[str] = []
    cursor = 0
    for fence in _FENCED_CODE_BLOCK_RE.finditer(content):
        prose, _ = redact_local_paths(content[cursor : fence.start()])
        parts.extend((prose, fence.group(0)))
        cursor = fence.end()
    prose, _ = redact_local_paths(content[cursor:])
    parts.append(prose)
    return "".join(parts)
...
def _assemble_bundle(
    ...
    *scrub_paths* runs ``redact_local_paths`` over every visible body, turning a
    ...
    for m in all_messages:
        ...
        # turns here the way the credential scrub above does. The tunnel send
        # passes False (trusted peer), so this is skipped there.
        if scrub_paths:
            content = _redact_file_export_message_paths(content)
