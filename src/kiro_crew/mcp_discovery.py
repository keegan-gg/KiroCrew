...
def _fix_stale_managed_command(name: str, spec: dict) -> None:
    ...
    needs ``["-s", "-m", "kiro_crew", <sub>]``, so re-resolving the command
...
def _is_first_party_managed_argv(
    ...
    *env* must equal the package-derived managed env too
    ...
    # Refuse the interpreter fallback (``<python> -s -m kiro_crew <sub>``):
    # ``-s`` disables the user site but does not remove the child's CWD from
    # sys.path. This package supports 3.10, so ``-P``/PYTHONSAFEPATH cannot be
    # assumed; a planted ``kiro_crew/`` tree in the gateway's cwd could still
    # shadow the installed package and run unconfined. Only a resolved
    # console-script binary, whose entrypoint imports from its own install,
    # qualifies. Keep recognizing the legacy flagless fallback defensively.
