...
#: and the ``auto/`` prefix, so none can hold a space or a parenthesis. A bare
#: ``none`` collided with a skill literally called ``none``, which made picking
#: that skill indistinguishable from declining to pick one.
...
def selected_skills(
    ...
    *,
    ...
    try:
        ...
        if cap <= 0:
            return None
        candidates = candidates_from_loader(skills_loader, text, project_dir)
        if not candidates:
            return None
