...
_CREW_READONLY_LEAVES: tuple[str, ...] = (
    ...
    "cloud.json",
    # Its cross-process advisory lock, sealed for the reason
    # ``agent_model_state.json.lock`` is: unsealed, a sandboxed process can unlink and
    # recreate it, so two writers flock DIFFERENT inodes and the loser's read-modify-write
    # erases the record the winner just made -- the lock file's IDENTITY is what makes the
    # lock a lock, and none of the field-level checks in the writer can see that it changed.
    # No sandboxed process legitimately takes it: every locker (``apply_update`` from the
    # wizard, the CLI and the dashboard handler) runs unsandboxed. Absent-file coverage is
    # the pre-create list below, because a name nothing occupies is a name an agent creates.
    "cloud.json.lock",
    # Where a write moves the bytes it DISPLACES from ``cloud.json`` -- the only copy of a
    # configuration that would otherwise be destroyed (an unparseable hand-edit, or the
    # newest content when a competing writer wins every bounded retry). Preserving it beside
    # the config left it in a writable parent, so the same sandboxed process the seal above
    # stops from editing the config could edit or delete its only recoverable copy, which is
    # the loss reintroduced one name over. A DIRECTORY rather than a fixed leaf because the
    # copies must not collide: one fixed name means a second corruption destroys the first
    # preservation, so each copy takes an unguessable name and the sealed parent is what
    # protects them all. Read-only, not hidden, for the reason ``cloud.json`` is readable:
    # the same in-sandbox readers may see it, and it holds nothing the config does not.
    "cloud-config-preserved",
...
_CREW_NOFOLLOW_READONLY_DIR_LEAVES: tuple[str, ...] = ("playwright-cli", "cloud-config-preserved")
...
#: derivation covers one SHAPE and silently leaves the other uncovered: the directory leaves
#: were omitted exactly that way, so a workspace set to the preservation directory kept the
#: write the seal denies. The assert below makes drift loud in both directions -- a sealed
#: leaf with no wording, and wording for a leaf nothing seals.
_DELEGATED_OVERLAP_LEAF_REASONS: "dict[str, tuple[str, str]]" = {
    ...
    "cloud-config-preserved": (
        "preserved cloud configurations",
        "the agent could edit or delete the only remaining copy of a configuration a write "
        "had to move aside, which is the loss sealing the configuration prevents",
    ),
    "playwright-cli": (
...
_CREW_PRECREATE_READONLY_FILE_LEAVES: tuple[str, ...] = (
    ...
    "cloud.json",
    # The lock beside it. Pre-created EMPTY, which is the whole content a flock target ever
    # needs: the file is a name to lock, never a document anything reads, so a zero-byte
    # pre-creation cannot be narrower or wider than the truth the way a config can.
    "cloud.json.lock",
...
def _refuse_aliased_strict_leaves() -> None:
    """Refuse an aliased strict leaf, on any platform, before the sandbox starts.

    The platform-independent half of the strict no-alias rule, split out so the Linux and
    macOS launchers cannot drift on it. Linux reached this only through
    :func:`_materialize_sealable_ceilings`, whose other job is preparing bind-mount
    targets, so the macOS launcher had no way to call the alias check without also
    calling Linux-specific work -- and it therefore did not call it at all. The refusal
    itself never depended on the sealing mechanism: it is a fact about the NAME, and both
    platforms need it.

    Both isolation mechanisms are name-based and neither follows a link for you. A
    read-only bind seals a mount, and a Seatbelt ``deny file-write*`` matches a pathname;
    in each case the alias reaches the same inode by a name the rule does not cover.
    """
