...
contextBridge.exposeInMainWorld("kirocrew", {
  ...
  // The transient shell pages (loading.html) also send `close` through it on
  // every platform, since they can be painted into a window whose native
  // close control is hidden. The action vocabulary and the per-platform
  // admission are validated in the main process (handleWindowControl /
  // applyWindowControl).
