...
// from a transient shell page off Linux. These tests pin both halves.
...
// not know): closing a connection window destroys it and its connect loop.
const CONNECTION_WINDOW_HINT =
  "This connection window closes and stops trying to connect. Kiro Crew itself keeps running.";
...
describe("window-control admission for the transient shell pages", () => {
  ...
  it("the token prompt is a transient shell page too", () => {
  ...
  it("a sender whose URL cannot be read is refused (fail closed)", () => {
    const sender = { getURL: () => { throw new Error("destroyed"); } };
    const { win, lifecycle } = lifecycleFor({ platform: "darwin", sender });
    lifecycle.chrome.windowControl(sender, "close");
    assert.deepEqual(win.calls, []);
  });
  ...
  it("a sender that is not this window's view is ignored", () => {
    const sender = fakeSender(SPLASH);
    const { win, lifecycle } = lifecycleFor({ platform: "darwin", sender });
    lifecycle.chrome.windowControl(fakeSender(SPLASH), "close");
    assert.deepEqual(win.calls, []);
  });
...
describe("loading.html carries its own close control", () => {
  it("has an accessible close button that is excluded from the drag region", () => {
    assert.match(
      LOADING_HTML,
      /<button id="closeWin" type="button" aria-describedby="closeHint" aria-label="Close window\. [^"]+">Close window<\/button>/,
  ...
  it("tells the user the app keeps running and what closing does to this window", () => {
    ...
    assert.equal(label[1], `Close window. ${CONNECTION_WINDOW_HINT}`);
  });
...
describe("loading.html words the close hint for the window and platform it is in", () => {
  // closeHintFor and its TRAY_LOCATION table, evaluated as written.
  const block = LOADING_HTML.match(/var TRAY_LOCATION = \{[\s\S]*?\};\s*function closeHintFor\([\s\S]*?\n    \}/);
  assert.ok(block, "loading.html must define TRAY_LOCATION and closeHintFor");
  const context = {};
  vm.runInNewContext(`${block[0]}; this.closeHintFor = closeHintFor;`, context);
  const { closeHintFor } = context;

  const MAIN_PREFIX = "Kiro Crew keeps running in the background and keeps trying to connect. Bring the window back from ";
  ...
  // The page must apply the sentence to what the user sees and hears: the
  // visible hint, and the button's aria-label, from the bridge's platform
  // and the primary marker. Fail-safe: primary starts false, so a splash with
  // no marker keeps the connection-window wording.
  it("renders the chosen sentence into the hint and the aria-label", () => {
    ...
    assert.match(LOADING_HTML, /closeWin\.setAttribute\("aria-label", "Close window\. " \+ hint\);/);
