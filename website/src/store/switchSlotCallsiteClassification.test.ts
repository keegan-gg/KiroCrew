...
const PINNED: Record<string, Counts> = {
  ...
  'src/pages/chat/useChatPageSessionController.ts': { announced: 2, keepTarget: 0, plain: 7, reason: 'tab-strip select + foreground open-in-tab announce; the plain sites are close-tab successor selection, URL deep-link restore (x2), late-frame deep-link recovery, mount re-sync, tab fallback and app-launch activation with its own sidError notice — programmatic, not gestures on a listed reference' },
