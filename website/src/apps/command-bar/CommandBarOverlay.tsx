...
type Scope = null | 'sessions' | 'artifacts'
...
function actionLabel(slot: Slot): string {
  switch (slot.tag) {
    ...
    case 'result':
      // An artifact row and a session row are the same SHAPE and a different
      // promise, and the footer names the promise. Keyed off the producing provider
      // rather than the live scope because this function is pure over the slot — and
      // because the row carries where it came from, while the scope is state next to
      // it that a future view could disagree with.
      return slot.row.providerId === 'artifacts'
        ? i18nT('apps.commandBar.action_open_artifact')
        : i18nT('apps.commandBar.action_open_session')
    case 'fallback':
...
export default function CommandBarOverlay({
      ...
      createArtifactsProvider({
        fetchArtifacts: q =>
          queryClient.fetchQuery<ArtifactsResponse>({
            queryKey: ['command-bar', 'artifacts', 'name', q],
  ...
  const { data: scopedResults, isFetching, isError, error: sessionsError, refetch: refetchSessions } = useQuery({
  ...
  const {
    ...
    // Distinct from the provider's own `['command-bar', 'artifacts', 'name', q]`
    // key, which caches the RESPONSE. This one caches the mapped rows, and the two
    // must not share a prefix segment count or the cache holds two different shapes
    // under keys that read as the same query.
    queryKey: ['command-bar', 'artifacts-view', artifactsQuery],
  ...
  const slots: Slot[] = useMemo(() => {
    ...
    return out
  }, [argCommand, isError, isFetching, query, ranked, recentRows, scope, scopedResults, searchArmed, artifactSlotRows, artifactsError, artifactsFetching, artifactsQuery])
  ...
  const activateIndex = useCallback(
    (index: number) => {
      ...
      switch (slot.tag) {
        ...
        case 'recovery':
          ...
          return
        case 'retry':
          // Routed by scope rather than by a flag on the slot: the row means "ask
          // again", and which engine it asks is a property of where the user is.
          if (scope === 'artifacts') void refetchArtifacts()
    ...
    [activateRoot, enterScope, navigate, onClose, pendingRow, query, refetchArtifacts, refetchSessions, scope, seedNewSession, slots],
  )
  ...
  const navName = argCommand
    ...
    : scope === 'sessions'
      ...
      : scope === 'artifacts'
        ...
        : ''
  const searchFailed =
    (scope === 'sessions' && searchArmed && isError) ||
    (scope === 'artifacts' && artifactsError)
  const searchError = scope === 'sessions' ? sessionsError : artifactError
  // Keep the concrete rejection as `message`: ErrorNotice uses an exact message
  // match to recover the endpoint, status and backend code from the error journal.
  const searchErrorDetail = searchFailed ? errMessage(searchError) : ''
      ...
      >
        ...
        {searchFailed && (
          <div className="px-3 py-2 border-b border-border">
            <ErrorNotice
              title={searchErrorDetail ? i18nT('apps.commandBar.search_failed') : undefined}
              message={searchErrorDetail || i18nT('apps.commandBar.search_failed')}
              variant="inline"
            />
          </div>
        )}
