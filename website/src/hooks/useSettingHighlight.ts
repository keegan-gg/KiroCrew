...
const LEGACY_ID_EXACT: Record<string, string> = {
  ...
  // Mode" to "Crew Members" when Crew Mode retired, then to "Agents" when the
  // crew-member vocabulary was dropped, then to "Agent Mode"; the flag and the
  // card are the same ones, only the label (and so the id) moved.
  ...
  'developer.agents': SETTINGS_CREW_MEMBERS_PREVIEW_ID,
}
