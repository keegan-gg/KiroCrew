 ...
 *   fe-03-detail      payments-platform detail: repo, sessions, no card for
 *                     anything the manifest does not carry (no MCP, no memory)
 ...
 *                     notice AND marked "Clone failed" inside the Repositories card
 ...
 *                     with the "Ask the agent" hand-off beside the blocked note
 ...
 *   fe-11-local-copy  detail scrolled to the Local copy card with the
 *                     "Remove from Kiro Crew" danger action in view
 *   fe-12-remove-confirm the Remove confirm dialog, naming the Project and what
 *                     removal keeps
 ...
 *
 * fe-01..fe-12 and fe-17 drive the isolated entry. fe-13..fe-16 drive the SHIPPED SPA
...
// id and the Repositories card row carries the "Clone failed" badge.
...
async function open(projects, { theme = 'dark', route = '/project-bundles', reviewPreview = null, clipboardFails = false } = {}) {
...
// fe-05 — sources_unavailable
{
  ...
  check('fe-05 notice leads with the action', /^Fix the source in project\.yaml, then sync\./.test((notice ?? '').trim()), 'first sentence is the fix, naming the file')
  ...
  const rowBadge = await infraRow.getByText('Clone failed', { exact: true }).count()
  check('fe-05 clone-failed badge on the repo row', rowBadge === 1, `Clone failed badge inside the infra row (count=${rowBadge})`)
  const staleBadge = await infraRow.getByText('Unavailable', { exact: true }).count()
  check('fe-05 row badge names the event, not a state', staleBadge === 0, `no "Unavailable" badge on the infra row (count=${staleBadge})`)
  const apiRow = page.locator('div.rounded-md', { hasText: 'https://github.com/acme/payments-api' })
  const apiBadge = await apiRow.getByText('Clone failed', { exact: true }).count()
  check('fe-05 healthy repo row unmarked', apiBadge === 0, `primary row carries no Clone failed badge (count=${apiBadge})`)
  const startDisabled = await page.getByRole('button', { name: 'New session' }).isDisabled()
...
// fe-11 — detail scrolled to the Local copy card
{
  ...
  check('fe-11 sync on the card', await page.getByRole('button', { name: 'Sync project' }).isVisible(), 'Sync project in the card header')
...
// fe-12 — the Remove confirm dialog
{
  ...
  check('fe-12 body says what stays', await dialog.getByText('Folders you added stay on disk. Kiro Crew removes only storage it created for this project.').isVisible(), 'body names what removal keeps')
...
// fe-14 — the composer chip on a Project-bound session
{
  ...
  await page.getByText('Where do charge intents get their idempotency key today?').waitFor()
  const explanation = "Project: payments-platform. This session's Project sets the workspace."
